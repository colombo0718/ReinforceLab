...
> 前提：Google 帳號登入（OAuth）先完成，SSO 是教室系統的基礎建設。

**演化路線：** 單機模式 → 教室模式 → 競技模式 → SS 調度層接入

- [ ] **Phase 1：班級基礎建設（MVP）**
  老師建立班級 → 產生班級碼 → 學生輸入班級碼加入 → 老師看全班訓練進度。
  - 老師端：建班、指定遊戲、觀察各學生的 reward 曲線與訓練集數
  - 學生端：輸入班級碼加入、正常訓練（體驗與現在一樣，只是老師看得到）
  - 技術：純前端可行（班級狀態用 Cloudflare KV 或 D1 存），不依賴 MOSME API
  - 這是「雙向觀察共控平台」的第一層：老師看進度；共控是第二步

- [ ] **Phase 2：班級管理功能**
  - 老師統一指定遊戲（學生端自動載入指定環境）
  - 老師可暫停 / 重置全班訓練
  - 班級榜單（依總 reward / 訓練集數 / 最高單局得分排名）
  - 學生成果匯出（CSV，供老師評分用）

- [ ] **Phase 3：競技模式**
  - 學生組隊、班內賽程配對
  - 對戰結果回收、班級排名更新
  > 此階段即 SS（Showdown System）的教學場版本。
  > 核心分工：遊戲決定玩什麼；RR 決定 AI 怎麼玩怎麼觀察；SS 決定誰和誰玩、玩完怎麼驗證記錄。

- [ ] **MOSME 嵌入**
  教室系統 Phase 1 完成後，即可與 MOSME 洽談嵌入：
  - MOSME 那邊放連結或 iframe，不依賴 MOSME API
  - 成績是否回寫 MOSME 成績管理系統待與范總確認
  > 嵌入後接現成技職學校教師/學生流量，無需自建推廣渠道。

- [ ] **SS 接入（長期）**
  SS 本質上是 RR 教室系統的 API 客戶端：
  SS 自動建房 → 強制把用戶推進 RR 教室模式 → 遊戲跑完 → SS 回收結果更新天梯。
  | | 一般教室模式 | SS 比賽模式 |
  |---|---|---|
  | 教室建立者 | 老師手動建 | SS 系統自動建 |
  | 加入方式 | 學生輸入班級碼 | SS 登入後直接推送 |
  | 房間性質 | 學習用 | 對戰用 |
  > 需等 RR 教室模式 Phase 1 完成後才能實作 SS 接入。
